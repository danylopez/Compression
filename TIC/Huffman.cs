...
namespace Huffman_namespace
{
    ...
    {
        private readonly Dictionary<T, NodoHuffman<T>> _DiccionarioHojas = new Dictionary<T, NodoHuffman<T>>();
        private readonly NodoHuffman<T> _raiz;

        //Creacion de codigo
        public Huffman(IEnumerable<T> valores)
        {
            var contador = new Dictionary<T, int>();
            var PrioridadCola = new PrioridadCola<NodoHuffman<T>>();
            int valorContador = 0;
            ...
            {
                var nodo = new NodoHuffman<T>((double)contador[valor] / valorContador, valor);
            ...
            {
                NodoHuffman<T> hijoIzq = PrioridadCola.Pop();
                NodoHuffman<T> hijoDer = PrioridadCola.Pop();
                var padre = new NodoHuffman<T>(hijoIzq, hijoDer);
        ...
        public void metodoCodificador(T valor, List<int> codificando)
        {
            ...
            NodoHuffman<T> nodoCur = _DiccionarioHojas[valor];
